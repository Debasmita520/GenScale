...
dgen.exp <- function (x, alpha, lambda, log = FALSE)
{
  ...
  MCMC=function(alpha1int,alpha2int,lamdaint,sd1,sd2,sd3,X1,X2,alpha1,alpha2,lamda,n1,n2,c,N){
    ...
    for (i in 2:N) {
      ...
      A=(n1/lamdastr^2)*(1+((alpha1str*(alpha1str-1))/(alpha1str-2))*(trigamma(1)-trigamma(alpha1str-1)+(digamma(alpha1str-1)-digamma(1))^2))-((n1*alpha1str)/(lamdastr^2))*((trigamma(1)-trigamma(alpha1str))+(digamma(alpha1str)-digamma(1))^2)+(n2/lamdastr^2)*(1+((alpha2str*(alpha2str-1))/(alpha2str-2))*(trigamma(1)-trigamma(alpha2str-1)+(digamma(alpha2str-1)-digamma(1))^2))-((n2*alpha2str)/(lamdastr^2))*((trigamma(1)-trigamma(alpha2str))+(digamma(alpha2str)-digamma(1))^2)
      
      p11=sqrt((n1/alpha1str^2)*(((n2*A)/alpha2str^2)+((n2^2)/(lamdastr^2))*((alpha2str/(alpha2str-1))*(digamma(alpha2str)-digamma(1))-(digamma(alpha2str+1)-digamma(1)))^2)+((n2*n1^2)/(alpha2str^2*lamdastr^2))*(((alpha1str/(alpha1str-1))*(digamma(alpha1str)-digamma(1))-(digamma(alpha1str+1)-digamma(1)))^2))
      
      B=(n1/lamdaint^2)*(1+((alpha1int*(alpha1int-1))/(alpha1int-2))*(trigamma(1)-trigamma(alpha1int-1)+(digamma(alpha1int-1)-digamma(1))^2))-((n1*alpha1int)/(lamdaint^2))*((trigamma(1)-trigamma(alpha1int))+(digamma(alpha1int)-digamma(1))^2)+(n2/lamdaint^2)*(1+((alpha2int*(alpha2int-1))/(alpha2int-2))*(trigamma(1)-trigamma(alpha2int-1)+(digamma(alpha2int-1)-digamma(1))^2))-((n2*alpha2int)/(lamdaint^2))*((trigamma(1)-trigamma(alpha2int))+(digamma(alpha2int)-digamma(1))^2)
      
      p12=sqrt((n1/alpha1int^2)*(((n2*A)/alpha2int^2)+((n2^2)/(lamdaint^2))*((alpha2int/(alpha2int-1))*(digamma(alpha2int)-digamma(1))-(digamma(alpha2int+1)-digamma(1)))^2)+((n2*n1^2)/(alpha2int^2*lamdaint^2))*(((alpha1int/(alpha1int-1))*(digamma(alpha1int)-digamma(1))-(digamma(alpha1int+1)-digamma(1)))^2))
      
      
      si1=prod(w11)*prod(w21)*p11
      si2=prod(w12)*prod(w22)*p12
      A=si1/si2
      si0=min(1,A) 
      u=runif(1)
  ...
  n2=length(x2)
    ...
    mcmc=MCMC(alpha1int=1.84,alpha2int=1.61,lamdaint=0.004,sd1=0.3,sd2=0.3,sd3=0.3,X1=x1,X2=x2,alpha1=alpha1,alpha2=alpha2,lamda=lamda,n1=n1,n2=n2,c=c,N=N)
