...
MCMC=function(alpha1int,alpha2int,lamdaint,sd1,sd2,sd3,X1,X2,alpha1,alpha2,lamda,n1,n2,c,N){
  ...
  for (i in 2:N) {
    ...
    A=(n2/lamdastr^2)*(1+((alpha2str*(alpha2str-1))/(alpha2str-2))*(trigamma(1)-trigamma(alpha2str-1)+(digamma(alpha2str-1)-digamma(1))^2))-((n2*alpha2str)/(lamdastr^2))*((trigamma(1)-trigamma(alpha2str))+(digamma(alpha2str)-digamma(1))^2)
    B=(n2/lamdaint^2)*(1+((alpha2int*(alpha2int-1))/(alpha2int-2))*(trigamma(1)-trigamma(alpha2int-1)+(digamma(alpha2int-1)-digamma(1))^2))-((n2*alpha2int)/(lamdaint^2))*((trigamma(1)-trigamma(alpha2int))+(digamma(alpha2int)-digamma(1))^2)
    p11=sqrt(n1*(((n2*A)/alpha2str)+((n2^2)/(lamdastr^2))*((alpha2str/(alpha2str-1))*(digamma(alpha2str)-digamma(1))-(digamma(alpha2str+1)-digamma(1)))^2)+((n2*n1^2)/(alpha2str^2*lamdastr^2))*(0.639996))
    p12=sqrt(n1*(((n2*B)/alpha2int)+((n2^2)/(lamdaint^2))*((alpha2int/(alpha2int-1))*(digamma(alpha2int)-digamma(1))-(digamma(alpha2int+1)-digamma(1)))^2)+((n2*n1^2)/(alpha2int^2*lamdaint^2))*(0.639996))
